...
for i in range(len(usernames)):
    user = User(username= usernames[i],passwordHash=hash,mail=mails[i],address=address)
    db.session.add(user)
    db.session.commit()
    announce = Announce(author=user.idUser, content=content_i, categorie="cuisine", creationDate=date.today(), finished=False)
